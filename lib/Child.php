...
abstract class Child implements \ArrayAccess
{
  ...
  /**
   * A list of column which are stored as JSON strings.
   */
  protected $json_fields = [];

  protected $json_encode_flags = 0;
  protected $json_encode_depth = JSON_DEFAULT_DEPTH;
  protected $json_decode_flags = 0;
  protected $json_decode_assoc = true;
  protected $json_decode_depth = JSON_DEFAULT_DEPTH;
    ...
    else
    {
      ...
      { // Inline JSON content.
        $value = json_encode($value, 
          $this->json_encode_flags,
          $this->json_encode_depth);
      }
  ...
  public function __get ($field)
  {
    ...
    $meth = "_get_$field";
    if (is_callable([$this, $meth]))
    { // Custom getter overrides all else.
      return $this->$meth();
    }
    else
    {
      $value = $this->data[$name] ?? null;
      if (isset($value) && in_array($name, $this->json_fields))
      { // The value is assumed to be encoded json.
        $value = json_decode($value,
          $this->json_decode_assoc, 
          $this->json_decode_depth,
          $this->json_decode_flags);
      }
      return $value;
    }
  }
